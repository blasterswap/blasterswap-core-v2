...
import "./BlasterswapV2ERC20.sol";
import "./libraries/Math.sol";
import "./libraries/UQ112x112.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/IBlasterswapV2Factory.sol";
import "./interfaces/IBlasterswapV2Callee.sol";
...
contract BlasterswapV2Pair is IBlasterswapV2Pair, BlasterswapV2ERC20 {
    ...
    function initialize(
        ...
        require(
            address(0) != _governor,
            "BlasterswapV2: governor is zero address"
        );
    ...
    function mint(
        address to
    ) external lock claimYield returns (uint liquidity) {
    ...
    function burn(
        address to
    ) external lock claimYield returns (uint amount0, uint amount1) {
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "BlasterswapV2: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "BlasterswapV2: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "BlasterswapV2: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint balance1;
        {
            ...
            require(
                ...
                "BlasterswapV2: INVALID_TO"
            );
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                IBlasterswapV2Callee(to).blasterswapV2Call(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(
            ...
            "BlasterswapV2: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
